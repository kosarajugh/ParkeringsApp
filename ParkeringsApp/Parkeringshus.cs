...
namespace ParkeringsApp
{
    public class Parkeringshus
    {
        ...
        // Visa parkeringens status med färgkodade platser
        public void VisaParkering()
        {
            Console.Clear();
            Console.WriteLine("Parkeringshus Status:");

            char rad = 'A'; // Starta med rad A
            for (int i = 0; i < TotalaPlatser; i++)
            {
                ...
                {
                    Console.ForegroundColor = ConsoleColor.Red;  // Upptagen plats
                    Console.Write($"[{rad}{(i % 5) + 1}] ");
                }

                // Var femte plats, gör ett radbyte
                if ((i + 1) % 5 == 0)
                {
                    Console.WriteLine();
                }
